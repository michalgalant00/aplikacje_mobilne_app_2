...
class PhoneDatabase {
  ...
  static Future<List<Phone>?> getPhones() async {
    final phoneDatabase = await openPhoneDatabase();
    final List<Map<String, dynamic>> phoneMapList = await phoneDatabase.query(
        phonesTableName); // konwertuje List<Map<String, dynamic> z bazy na List<Word>.

    if (phoneMapList.isNotEmpty) {
      return List.generate(
          phoneMapList.length, (index) => Phone.fromJson(phoneMapList[index]));
    }
